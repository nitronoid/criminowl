...
float turb (vec3 _pos, float _frequency)
{
    ...
    for(int i = 0; i < 8; ++i)
	{
        ret += abs(snoise(pos*frequency))/frequency;
        frequency*=2.1;
	}
    return ret;
}
...
{
    vec3 pos = _pos;
    pos += snoise(pos*frequency)/frequency;
    pos *= stretch;
...
{
    return _pos + snoise(_pos + rand) * scale;
}

vec3 randPos(vec3 _pos, float rand)
{
    return _pos + snoise(_pos + rand);
}

float blendNoise(vec3 pos, float freq)
{
    return clamp(snoise(randPos(pos,0,1) * freq)/freq, 0.0, 1.0);
}